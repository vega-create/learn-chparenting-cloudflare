...
function getUnitSeoData(uid: number): UnitSEOInput | undefined {
  const u = INTER_UNITS.find(x => x.id === uid);
...
export default async function Page({ params }: Props) {
  const { id } = await params;
  const uid = Number(id);
  const unit = INTER_UNITS.find(u => u.id === uid);
    ...
    <>
      ...
      {unit && seo && (
        <UnitSEOContent
          ...
          prevUnit={INTER_UNITS.find(u => u.id === uid - 1) && { id: uid - 1, title: INTER_UNITS.find(u => u.id === uid - 1)!.title }}
          nextUnit={INTER_UNITS.find(u => u.id === uid + 1) && { id: uid + 1, title: INTER_UNITS.find(u => u.id === uid + 1)!.title }}
        />
      )}
    </>
  );
