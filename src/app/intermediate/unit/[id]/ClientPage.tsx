...
UNITS.forEach((u: any, ui: number) => {
  u.vocab.forEach((v: any, vi: number) => {
    ...
    if (v.ex) exAudioMap.set(v.ex, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/intermediate/unit${ui + 1}/ex-${vi + 1}-normal.mp3?v1`);
  });
    ...
    <div>
      ...
      <div className="text-white py-4 md:py-6 px-4" style={{ background: unit.color }}>
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          <button onClick={() => router.push("/intermediate")}
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 flex justify-between items-center gap-3">
        {prevUnit ? (
          <button onClick={() => { router.push(`/intermediate/unit/${prevUnit.id}`); setTab("vocab"); }}
        ...
        {nextUnit ? (
          <button onClick={() => { router.push(`/intermediate/unit/${nextUnit.id}`); setTab("vocab"); }}
          ...
          <a href="/intermediate/game" className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-500 text-white text-sm font-bold no-underline hover:bg-amber-600 transition">
            🎮 去遊戲練習 →
          </a>
        )}
      </div>
