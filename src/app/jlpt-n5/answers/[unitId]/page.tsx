...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `日文 N5${unit?.title || ""}練習單答案與詳細解析，幫助孩子理解每一題的解題思路。`,
  };
}
...
export default async function Page({ params }: Props) {
  ...
  if (!unit) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        ...
        <a href="/jlpt-n5" className="text-blue-600 hover:underline mt-4 inline-block">← 返回日文 N5</a>
      </div>
    );
  }
  ...
  const nextNum = num + 1;
  const hasNext = N5_UNITS.some((u) => u.id === nextNum);
  const nextUnitUrl = hasNext ? `/jlpt-n5/answers/unit-${String(nextNum).padStart(2, "0")}` : undefined;
