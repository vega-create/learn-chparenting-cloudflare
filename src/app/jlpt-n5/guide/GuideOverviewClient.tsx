    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="mt-4 mb-6">
        ...
        <p className="text-slate-500 text-sm">
          不知道怎麼陪孩子學？30 秒看完就知道今天陪什麼、怎麼陪、要多久。
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl p-5 border border-indigo-200 mb-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button
            ...
            className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold text-sm cursor-pointer border-none hover:bg-indigo-700 transition"
          >
            免費索取
          </button>
        </div>
      ...
      <div className="space-y-3">
        {units.map((u) => (
          <a
            key={u.id}
            href={u.hasGuide ? `${basePath}/guide/unit-${String(u.id).padStart(2, "0")}` : undefined}
            className={`block bg-white rounded-xl p-4 border border-slate-200 shadow-sm no-underline transition ${
              u.hasGuide ? "hover:border-indigo-300 hover:shadow-md cursor-pointer" : "opacity-50"
            }`}
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {u.learningGoal && (
                  <div className="text-xs text-slate-500 mt-0.5 truncate">{u.learningGoal}</div>
                )}
              </div>
