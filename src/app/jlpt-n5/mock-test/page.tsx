...
const allListening = N5_UNITS.flatMap(u => u.listening);
const allQuiz = N5_UNITS.flatMap(u => u.quiz);
const allReading = N5_UNITS.flatMap(u => Array.isArray(u.reading) ? u.reading : [u.reading]);
...
export default function MockTestPage() {
  ...
  useEffect(() => {
    if (phase === "result" && !mockTracked.current) {
      ...
      trackActivity({
        subject: "jlpt", activityType: "mock-test", activityId: "jlpt-n5-mock",
        activityName: "JLPT N5 模擬測驗", score: correct, maxScore: total,
  ...
  if (phase === "intro") return (
    <div className="max-w-2xl mx-auto px-4 py-12 text-center">
      <div className="bg-white rounded-2xl p-8 md:p-10 border border-slate-200 shadow-sm">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-slate-800 mb-3">JLPT N5 模擬測驗</h1>
        <p className="text-slate-500 mb-6">日本語能力試驗 N5 級模擬考（考完看成績與答案）</p>
        ...
        <div className="mt-4"><a href="/jlpt-n5" className="text-sm text-blue-500 hover:underline">← 返回 JLPT N5 首頁</a></div>
      </div>
    </div>
  );
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-10 text-center">
        <div className="bg-white rounded-2xl p-8 md:p-10 border border-slate-200 shadow-lg">
          ...
          <ShareButtons text={`我在 JLPT N5 模擬考答對了 ${correct}/${total} 題！你也來試試 💪`} url="/jlpt-n5/mock-test" />
          <div className="flex gap-3 justify-center flex-wrap mt-4">
            ...
            <a href="/jlpt-n5" className="px-6 py-2.5 rounded-xl border-2 border-rose-300 text-rose-400 font-semibold text-sm no-underline hover:bg-rose-50 transition">← 返回</a>
          </div>
        </div>
      </div>
    );
