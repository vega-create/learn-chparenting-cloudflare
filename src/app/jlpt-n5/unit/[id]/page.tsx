...
function getUnitSeoData(uid: number): UnitSEOInput | undefined {
  const u = N5_UNITS.find(x => x.id === uid);
    ...
    <>
      ...
      {unit && seo && (
        <UnitSEOContent
          ...
          prevUnit={N5_UNITS.find(u => u.id === uid - 1) && { id: uid - 1, title: N5_UNITS.find(u => u.id === uid - 1)!.title }}
          nextUnit={N5_UNITS.find(u => u.id === uid + 1) && { id: uid + 1, title: N5_UNITS.find(u => u.id === uid + 1)!.title }}
        />
      )}
    </>
  );
