...
export default function JlptUnitPage() {
  ...
  const unit = N4_UNITS.find(u => u.id === uid);
  ...
  const nextUnit = N4_UNITS.find(u => u.id === uid + 1);
  const prevUnit = N4_UNITS.find(u => u.id === uid - 1);

  return (
    <div>
      <div className="text-white py-4 md:py-6 px-4" style={{ background: unit.color }}>
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          <button onClick={() => router.push("/jlpt-n4")}
      ...
      <div className="max-w-4xl mx-auto px-3 md:px-4 pt-3">
        <WorksheetActions toolSlug="japanese" level="n4" unitId={unitId} unitName={unit.title} color="purple" />
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 flex justify-between items-center gap-3">
        {prevUnit ? (
          <button onClick={() => { router.push(`/jlpt-n4/unit/${prevUnit.id}`); setTab("vocab"); }}
        ...
        {nextUnit ? (
          <button onClick={() => { router.push(`/jlpt-n4/unit/${nextUnit.id}`); setTab("vocab"); }}
          ...
          <a href="/jlpt-n4/game" className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-500 text-white text-sm font-bold no-underline hover:bg-amber-600 transition">
            🎮 去遊戲練習 →
          </a>
        )}
      </div>
...
function SpeakingTab({ unit }: { unit: JlptUnit }) {
  ...
  const normalizeJa = (s: string) => {
    let t = s.replace(/[。、！？「」（）\s・～〜]/g, "");
    // 常見漢字↔假名等價（語音辨識常回傳漢字形式）
    const wordMap: [RegExp, string][] = [
      [/私/g, "わたし"], [/僕/g, "ぼく"], [/俺/g, "おれ"],
      [/彼/g, "かれ"], [/彼女/g, "かのじょ"],
      [/今日/g, "きょう"], [/明日/g, "あした"], [/昨日/g, "きのう"],
      [/今/g, "いま"], [/何/g, "なに"], [/誰/g, "だれ"],
      [/友達/g, "ともだち"], [/先生/g, "せんせい"], [/学生/g, "がくせい"],
      [/学校/g, "がっこう"], [/会社/g, "かいしゃ"], [/仕事/g, "しごと"],
      [/食べ/g, "たべ"], [/飲み/g, "のみ"], [/行き/g, "いき"], [/来/g, "き"],
      [/見/g, "み"], [/聞き/g, "きき"], [/読み/g, "よみ"], [/書き/g, "かき"],
      [/買い/g, "かい"], [/使い/g, "つかい"], [/作り/g, "つくり"],
      [/大きい/g, "おおきい"], [/小さい/g, "ちいさい"],
      [/新しい/g, "あたらしい"], [/古い/g, "ふるい"],
      [/高い/g, "たかい"], [/安い/g, "やすい"],
      [/良い/g, "よい"], [/悪い/g, "わるい"],
      [/早い/g, "はやい"], [/遅い/g, "おそい"],
      [/父/g, "ちち"], [/母/g, "はは"], [/兄/g, "あに"], [/姉/g, "あね"],
      [/弟/g, "おとうと"], [/妹/g, "いもうと"],
      [/お父さん/g, "おとうさん"], [/お母さん/g, "おかあさん"],
      [/お兄さん/g, "おにいさん"], [/お姉さん/g, "おねえさん"],
      [/家族/g, "かぞく"], [/家/g, "いえ"], [/人/g, "にん"],
      [/日本/g, "にほん"], [/日本語/g, "にほんご"],
      [/英語/g, "えいご"], [/中国語/g, "ちゅうごくご"],
      [/電車/g, "でんしゃ"], [/電話/g, "でんわ"],
      [/時間/g, "じかん"], [/時/g, "じ"], [/分/g, "ふん"],
      [/天気/g, "てんき"], [/元気/g, "げんき"],
    ];
    for (const [re, kana] of wordMap) t = t.replace(re, kana);
    // 漢字數字→阿拉伯（常見）
    ...
    // 發音等價
    t = t.replace(/ぐ/g, 'く').replace(/グ/g, 'ク');
    t = t.replace(/づ/g, 'ず').replace(/ヅ/g, 'ズ');
    t = t.replace(/ぢ/g, 'じ').replace(/ヂ/g, 'ジ');
    t = t.replace(/を/g, 'お').replace(/ヲ/g, 'オ');
    t = t.replace(/は([^行])/g, 'わ$1');
    // 全形→半形數字
          ...
          <div>
            ...
            <div className="mt-6 space-y-3 text-center">
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <a href={`/worksheets/japanese/n4/unit-${String(unit.id).padStart(2,"0")}.pdf`} download
              ...
              <button onClick={() => setEbookOpen(true)}
                ...
                <p className="text-xs text-indigo-600 mt-1">免費索取《日文 N4家長陪伴指南》→ 立即索取</p>
              </button>
            </div>
          </div>
        )}
      </div>
      <EbookModal toolName="日文 N4" ebookSlug="japanese-n4" isOpen={ebookOpen} onClose={() => setEbookOpen(false)} />
    </div>
  );
}
