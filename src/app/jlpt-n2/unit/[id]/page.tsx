...
function getUnitSeoData(uid: number): UnitSEOInput | undefined {
  const u = N2_UNITS.find(x => x.id === uid);
    ...
    <>
      ...
      {unit && seo && (
        <UnitSEOContent
          ...
          prevUnit={N2_UNITS.find(u => u.id === uid - 1) && { id: uid - 1, title: N2_UNITS.find(u => u.id === uid - 1)!.title }}
          nextUnit={N2_UNITS.find(u => u.id === uid + 1) && { id: uid + 1, title: N2_UNITS.find(u => u.id === uid + 1)!.title }}
        />
      )}
    </>
  );
