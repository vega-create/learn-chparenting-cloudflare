...
export default async function Page({ params }: Props) {
  ...
  const num = getUnitNum(unitId);
  const unit = N2_UNITS.find((u) => u.id === num);
  const guide = JLPT_N2_GUIDES[num];

  if (!unit || !guide) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        <h1 className="text-2xl font-bold text-slate-800">找不到此指南</h1>
        <a href="/jlpt-n2/guide" className="text-indigo-600 hover:underline mt-4 inline-block">
          ← 返回日文 N2 指南總覽
        </a>
      </div>
    );
  ...
  const nextNum = num + 1;
  const hasNext = N2_UNITS.some((u) => u.id === nextNum) && !!JLPT_N2_GUIDES[nextNum];
  const nextGuideUrl = hasNext ? `/jlpt-n2/guide/unit-${String(nextNum).padStart(2, "0")}` : undefined;
  const pdfUrl = `/worksheets/japanese/n2/${unitId}.pdf`;
  const unitUrl = `/jlpt-n2/unit/${num}`;
