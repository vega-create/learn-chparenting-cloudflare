...
const allListening = N3_UNITS.flatMap(u => u.listening);
const allQuiz = N3_UNITS.flatMap(u => u.quiz);
const allReading = N3_UNITS.flatMap(u => Array.isArray(u.reading) ? u.reading : [u.reading]);
...
export default function MockTestPage() {
  ...
  useEffect(() => {
    if (phase === "result" && !mockTracked.current) {
      ...
      trackActivity({
        subject: "jlpt", activityType: "mock-test", activityId: "jlpt-n3-mock",
        activityName: "JLPT N3 模擬測驗", score: correct, maxScore: total,
  ...
  if (phase === "intro") return (
    <div className="max-w-2xl mx-auto px-4 py-12 text-center">
      <div className="bg-white rounded-2xl p-8 md:p-10 border border-slate-200 shadow-sm">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-slate-800 mb-3">JLPT N3 模擬測驗</h1>
        ...
        <div className="mt-4"><a href="/jlpt-n3" className="text-sm text-blue-500 hover:underline">← 返回 JLPT N3 首頁</a></div>
      </div>
    </div>
  );
