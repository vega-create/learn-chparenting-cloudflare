...
function getUnitSeoData(uid: number): UnitSEOInput | undefined {
  const u = N3_UNITS.find(x => x.id === uid);
    ...
    <>
      ...
      {unit && seo && (
        <UnitSEOContent
          ...
          prevUnit={N3_UNITS.find(u => u.id === uid - 1) && { id: uid - 1, title: N3_UNITS.find(u => u.id === uid - 1)!.title }}
          nextUnit={N3_UNITS.find(u => u.id === uid + 1) && { id: uid + 1, title: N3_UNITS.find(u => u.id === uid + 1)!.title }}
        />
      )}
    </>
  );
