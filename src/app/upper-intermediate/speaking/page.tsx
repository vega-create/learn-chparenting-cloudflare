...
UNITS.forEach((u: any, ui: number) => {
  u.vocab.forEach((v: any, vi: number) => {
    ...
    if (v.ex) sentenceAudioMap.set(v.ex, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/upper-intermediate/unit${ui + 1}/ex-${vi + 1}-normal.mp3?v1`);
  });
  u.listening.forEach((l: any, li: number) => {
    sentenceAudioMap.set(l.text, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/upper-intermediate/unit${ui + 1}/listen-${li + 1}.mp3?v1`);
  });
});
  ...
  const startRecording = (targetText: string, qaMode = false) => {
        ...
        for (let r = 0; r < e.results.length; r++) {
          if (e.results[r].isFinal && !collected[r]) {
            ...
            for (let a = 0; a < e.results[r].length; a++) {
              alts.push({ transcript: e.results[r][a].transcript, confidence: e.results[r][a].confidence ?? 0 });
            }
            collected[r] = alts;
          }
        }
    ...
    recog.onresult = (e: any) => {
      ...
      if (qaMode) {
        ...
        setResult({ transcript: bestAlt.transcript, pct: bestPct, matched: [], rawPct: 0, confidence: conf, isQa: true, wordCount });
      } else {
