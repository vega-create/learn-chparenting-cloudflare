...
export default async function Page({ params }: Props) {
  ...
  const num = getUnitNum(unitId);
  const unit = UI_UNITS.find((u) => u.id === num);
  const guide = GEPT_UPPER_INTERMEDIATE_GUIDES[num];

  if (!unit || !guide) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        <h1 className="text-2xl font-bold text-slate-800">找不到此指南</h1>
        <a href="/upper-intermediate/guide" className="text-indigo-600 hover:underline mt-4 inline-block">
          ← 返回 GEPT 中高級指南總覽
        </a>
      </div>
    );
  ...
  const nextNum = num + 1;
  const hasNext = UI_UNITS.some((u) => u.id === nextNum) && !!GEPT_UPPER_INTERMEDIATE_GUIDES[nextNum];
  const nextGuideUrl = hasNext ? `/upper-intermediate/guide/unit-${String(nextNum).padStart(2, "0")}` : undefined;
  const pdfUrl = `/worksheets/gept/upper-intermediate/${unitId}.pdf`;
  const unitUrl = `/upper-intermediate/unit/${num}`;
