...
// ElevenLabs mp3 lookup (upper-intermediate, R2)
...
UNITS.forEach((u: any, ui: number) => {
  u.vocab.forEach((v: any, vi: number) => {
    vocabAudioMap.set(v.en, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/upper-intermediate/unit${ui + 1}/word-${vi + 1}-normal.mp3?v1`);
    if (v.ex) exAudioMap.set(v.ex, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/upper-intermediate/unit${ui + 1}/ex-${vi + 1}-normal.mp3?v1`);
  });
  u.listening.forEach((l: any, li: number) => {
    listenAudioMap.set(l.text, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/upper-intermediate/unit${ui + 1}/listen-${li + 1}.mp3?v1`);
  });
  const readings = Array.isArray(u.reading) ? u.reading : (u.reading ? [u.reading] : []);
  readings.forEach((r: any, ri: number) => {
    if (r && r.passage) passageAudioMap.set(r.passage, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/upper-intermediate/unit${ui + 1}/passage-${ri + 1}.mp3?v1`);
  });
});
    ...
    <div>
      ...
      <div className="text-white py-4 md:py-6 px-4" style={{ background: unit.color }}>
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          <button onClick={() => router.push("/upper-intermediate")}
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 flex justify-between items-center gap-3">
        {prevUnit ? (
          <button onClick={() => { router.push(`/upper-intermediate/unit/${prevUnit.id}`); setTab("vocab"); }}
        ...
        {nextUnit ? (
          <button onClick={() => { router.push(`/upper-intermediate/unit/${nextUnit.id}`); setTab("vocab"); }}
          ...
          <a href="/upper-intermediate/game" className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-500 text-white text-sm font-bold no-underline hover:bg-amber-600 transition">
            🎮 去遊戲練習 →
          </a>
        )}
      </div>
          ...
          <div>
            ...
            <div className="mt-6 pt-5 border-t border-slate-100 text-left space-y-3">
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <a href={`/worksheets/gept/upper-intermediate/unit-${String(unit.id).padStart(2,"0")}.pdf`} download
                  className="inline-flex items-center gap-2 px-4 py-2.5 bg-rose-600 text-white rounded-lg font-bold text-sm no-underline hover:bg-rose-700 transition">
                  📥 下載練習單 (PDF)
                </a>
              </div>
              <button onClick={() => setEbookOpen(true)}
                ...
                <p className="text-xs text-indigo-600 mt-1">免費索取《GEPT 中高級家長陪伴指南》→ 立即索取</p>
              </button>
            </div>
          </div>
        )}
      </div>
      <EbookModal toolName="GEPT 中高級" ebookSlug="gept-upper-intermediate" isOpen={ebookOpen} onClose={() => setEbookOpen(false)} />
    </div>
  );
}
