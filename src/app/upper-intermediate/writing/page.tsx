"use client";
import { UI_WRITING as ELEM_WRITING } from "@/data/writing/upper-intermediate-writing";
...
function ResultScreen({ score, total, onBack }: { score: number; total: number; onBack: () => void }) {
  ...
  return (
    <div className="max-w-sm mx-auto mt-16 text-center bg-white rounded-2xl p-10 border border-slate-200 shadow-lg animate-slideUp">
      ...
      <ShareButtons text={`我在全民英檢中高級寫作練習得了 ${score} 分！✍️`} url="/upper-intermediate/writing" />
      <div className="mt-4">
  ...
  if (!tab) return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="text-center mt-6">
        <a href="/upper-intermediate" className="text-sm text-blue-500 hover:underline">← 回到中高級</a>
      </div>
    </div>
  );
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
            ...
            return (
              <div className="space-y-3">
                <div className={`text-center p-3 rounded-xl text-sm font-bold ${isCorrect ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>
                  {isCorrect ? "✅ Correct!" : `❌ Answer: ${q.parts.join(" ")}`}
                </div>
                {q.explanation && (
      ...
      {tab === "paragraph" && (
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 shadow-sm">
          ...
          {show && (() => {
            const isCorrect = paraArr.map((s: any) => s.t).join("|") === q.sentences.join("|");
            return (
              <div className="space-y-3">
                <div className={`p-4 rounded-xl text-sm ${isCorrect ? "bg-emerald-50 border border-emerald-200" : "bg-red-50 border border-red-200"}`}>
                  {isCorrect
