...
UNITS.forEach((u: any, ui: number) => {
  (u.listening || []).forEach((l: any, li: number) => {
    listenAudioMap.set(l.text, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/elementary/unit${ui + 1}/listen-${li + 1}.mp3?v1`);
  });
  const readings = Array.isArray(u.reading) ? u.reading : (u.reading ? [u.reading] : []);
  readings.forEach((r: any, ri: number) => {
    if (r && r.passage) passageAudioMap.set(r.passage, `https://pub-a36eb12da250439e9bdd35709d3d1cd4.r2.dev/elementary/unit${ui + 1}/passage-${ri + 1}.mp3?v1`);
  });
});
