...
/* ─── Data sources ─── */
const allVocab = N1_UNITS.flatMap(u => u.vocab);
const allSentences = N1_UNITS.flatMap(u => u.listening.map(l => ({ text: l.text, zh: l.zh })));
const allPassages = N1_UNITS.flatMap(u => {
  ...
  const startRecording = (targetText: string, qaMode = false) => {
    ...
    recog.onresult = (e: SREvent) => {
      ...
      if (qaMode) {
        ...
        const charCount = bestAlt.transcript.replace(/[\s。、！？「」（）\u3000・～〜…ー]/g, "").length;
        const clarityScore = Math.round(conf * 55);
        const completeScore = Math.min(45, Math.round((charCount / 15) * 45));
        bestPct = Math.min(100, clarityScore + completeScore);
        setResult({ transcript: bestAlt.transcript, pct: bestPct, matched: [], rawPct: 0, confidence: conf, isQa: true, charCount });
      } else {
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="grid gap-4">
        {[
          ...
          { m: "qa" as Mode, icon: "💬", title: "問答練習", desc: "聽日文問題、用日文回答", tag: "N4", color: "#dc2626", count: "5 題" },
        ].map(item => (
      ...
      <div className="text-center mt-6">
        <a href="/jlpt-n1" className="text-sm text-red-500 hover:underline no-underline">← 回到 JLPT N1</a>
      </div>
    </div>
  );
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-12 text-center">
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-lg">
          ...
          <ShareButtons text={`我在JLPT N1口說練習得了 ${avgScore}% 的成績！🎙️`} url="/jlpt-n1/speaking" />
              ...
              {result.isQa && (
                <div className="grid grid-cols-2 gap-2 mb-3">
                  ...
                  <div className="bg-purple-50 rounded-xl p-3 text-center">
                    ...
                    <div className="text-lg font-bold" style={{ color: (result.charCount || 0) >= 15 ? "#059669" : (result.charCount || 0) >= 8 ? "#f59e0b" : "#ef4444" }}>
                      {result.charCount || 0} 字
                    </div>
                  </div>
                </div>
              )}
