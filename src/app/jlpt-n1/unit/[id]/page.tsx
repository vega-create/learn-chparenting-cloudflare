...
export default function JlptUnitPage() {
  ...
  const nextUnit = N1_UNITS.find(u => u.id === uid + 1);
  const prevUnit = N1_UNITS.find(u => u.id === uid - 1);

  return (
    <div>
      <div className="text-white py-4 md:py-6 px-4" style={{ background: unit.color }}>
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          <button onClick={() => router.push("/jlpt-n1")}
      ...
      <div className="max-w-4xl mx-auto px-3 md:px-4 pt-3">
        <WorksheetActions toolSlug="japanese" level="n1" unitId={unitId} unitName={unit.title} color="amber" />
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 flex justify-between items-center gap-3">
        {prevUnit ? (
          <button onClick={() => { router.push(`/jlpt-n1/unit/${prevUnit.id}`); setTab("vocab"); }}
        ...
        {nextUnit ? (
          <button onClick={() => { router.push(`/jlpt-n1/unit/${nextUnit.id}`); setTab("vocab"); }}
          ...
          <a href="/jlpt-n1/game" className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-500 text-white text-sm font-bold no-underline hover:bg-amber-600 transition">
            🎮 去遊戲練習 →
          </a>
        )}
      </div>
          ...
          <div>
            ...
            <div className="mt-6 space-y-3 text-center">
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <a href={`/worksheets/japanese/n1/unit-${String(unit.id).padStart(2,"0")}.pdf`} download
              ...
              <button onClick={() => setEbookOpen(true)}
                ...
                <p className="text-xs text-indigo-600 mt-1">免費索取《日文 N1家長陪伴指南》→ 立即索取</p>
              </button>
            </div>
          </div>
        )}
      </div>
      <EbookModal toolName="日文 N1" ebookSlug="japanese-n1" isOpen={ebookOpen} onClose={() => setEbookOpen(false)} />
    </div>
  );
}
