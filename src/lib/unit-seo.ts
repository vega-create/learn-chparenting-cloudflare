...
export function generateUnitMetadata(level: LevelInfo, unit: UnitSEOInput | undefined): Metadata {
  ...
  const descriptionRaw = `${level.displayName} Unit ${unit.id}「${unit.title}」主題練習：${unit.vocabCount} 個重點單字（${vocabSnippet}…）、${unit.grammarCount} 個文法（${grammarSnippet}…）、${unit.listeningCount} 題聽力，含閱讀理解與測驗。免費線上練習，可下載練習單。`;
  // Bing 建議 description 在 160 字元內；中高級的單字片段較長會超過，超過就在最後一個頓號前收尾
  const description = descriptionRaw.length > 155 ? descriptionRaw.slice(0, 152).replace(/[、，,（(][^、，,（(]*$/, "") + "…" : descriptionRaw;
